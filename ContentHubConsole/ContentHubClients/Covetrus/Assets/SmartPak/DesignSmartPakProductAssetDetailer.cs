...
{
    internal class DesignSmartPakProductAssetDetailer : BaseDetailer
    {
        private const string GPM_CATALOG_NAME = "GPM-SmartPak Master Catalog";
        private const string BUSINESS_DOMAIN_NAME = "Business.Domain.SmartPak";

        public DesignSmartPakProductAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public override async Task AssignToProduct(CovetrusAsset asset, long businessDomainId)
        {
            ...
            var fnClean = filename.Split('.').First().Replace("~", "");

            var productNumbers = NormalSkuProcessing(fnClean);
            var products = await _productManager.GetProductByNumbers(productNumbers, businessDomainId);
        ...
        private List<string> NormalSkuProcessing(string filename)
        {
            char[] sepArray = new char[] { '_', ' ', '-' };
            var sp = filename.Split(sepArray);

            var current = new List<string>();
            var resultSkus = new List<string> { sp[0] };
            ...
            List<long> longSkus = new List<long>();
            foreach (var item in current.Where(w => w.Length >= 5).OrderBy(o => o))
            {
                var isLong = Int64.TryParse(item.Substring(0, 5), out long skuNum);
                if (isLong)
                {
                    longSkus.Add(skuNum);
                }
            ...
            {
                resultSkus.Clear();
                resultSkus.AddRange(longSkus.Select(s => s.ToString()).ToList());
            }
