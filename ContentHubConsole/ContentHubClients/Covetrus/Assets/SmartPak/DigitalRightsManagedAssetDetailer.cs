...
{
    public class DigitalRightsManagedAssetDetailer : BaseDetailer
    {
        public DigitalRightsManagedAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long imageId = _taxonomyManager.AssetTypeEntities.Where(w => w.Identifier.Equals("M.AssetType.StockImages")).FirstOrDefault().Id.Value;
            long imageUsageIds = _taxonomyManager.AssetUsageEntities.Where(w => w.Identifier.Equals("CV.AssetUsage.StockImages")).FirstOrDefault().Id.Value;
            ...
            foreach (var asset in _covetrusAsset)
            {
                try
                {
                    ...
                    asset.AddChildToManyParentsRelation(885, RelationNames.RELATION_DIGITAL_RIGHTS_RESTRICTED);
                    asset.AddChildToManyParentsRelation(32786, RelationNames.RELATION_DIGITAL_RIGHTS_PROFILE);
