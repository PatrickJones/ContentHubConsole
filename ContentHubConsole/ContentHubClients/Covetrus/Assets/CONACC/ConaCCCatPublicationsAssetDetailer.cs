...
{
    public class ConaCCCatPublicationsAssetDetailer : BaseDetailer
    {
        public ConaCCCatPublicationsAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long spPhotoBusinessDomainId = _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.Publications")).FirstOrDefault().Id.Value;
        ...
        internal void SetUsages(CovetrusAsset asset)
        {
            ...
            foreach (var s in orgSpilt)
            {
                ...
                if (s.Equals("Ads", StringComparison.InvariantCultureIgnoreCase) && !pathStrings.Any(a => a == "Advertising"))
                {
                    pathStrings.Add("Advertising");
                }
            }
