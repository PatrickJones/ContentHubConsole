...
{
    public class ConaCCCatCSAssetDetailer : BaseDetailer
    {
        public ConaCCCatCSAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long spPhotoBusinessDomainId = _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.CS")).FirstOrDefault().Id.Value;
