...
{
    internal class ConaCCCatCMPAssetDetailer : BaseDetailer
    {
        public ConaCCCatCMPAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long spPhotoBusinessDomainId = _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.CMP")).FirstOrDefault().Id.Value;
