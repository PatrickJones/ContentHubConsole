...
{
    internal class ConaCCCatTBSAssetDetailer : BaseDetailer
    {
        public ConaCCCatTBSAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long spPhotoBusinessDomainId = _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.TBS")).FirstOrDefault().Id.Value;
        ...
        private void SetMultipleBusinessDomains(CovetrusAsset asset)
        {
            ...
            if (typeFolderLevel != null)
            {
                ...
                {
                    _ when typeFolderLevel.Equals("Business Solutions", StringComparison.InvariantCultureIgnoreCase) => _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.BusinessSolutions")).FirstOrDefault().Id.Value,
                    _ when typeFolderLevel.Equals("Covetrus Care Plans", StringComparison.InvariantCultureIgnoreCase) => _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.CovetrusCarePlans")).FirstOrDefault().Id.Value,
                    _ when typeFolderLevel.Equals("GTS", StringComparison.InvariantCultureIgnoreCase) => _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.GTS")).FirstOrDefault().Id.Value,
                    _ when typeFolderLevel.Equals("Prescription Management", StringComparison.InvariantCultureIgnoreCase) => _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.PrescriptionManagement")).FirstOrDefault().Id.Value,
