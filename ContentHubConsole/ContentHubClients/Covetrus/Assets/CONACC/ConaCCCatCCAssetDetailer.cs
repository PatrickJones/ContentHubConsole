...
{
    public class ConaCCCatCCAssetDetailer : BaseDetailer
    {
        public ConaCCCatCCAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public async override Task<long> UpdateAllAssets()
        {
            ...
            long spPhotoBusinessDomainId = _taxonomyManager.BusinessDomainEntities.Where(w => w.Identifier.Equals("Business.Domain.CONACC.CC")).FirstOrDefault().Id.Value;
            long dropboxId = _taxonomyManager.MigrationOriginEntities.Where(w => w.Identifier.Contains("Dropbox")).FirstOrDefault().Id.Value;
            long imageId = _taxonomyManager.AssetTypeEntities.Where(w => w.Identifier.Equals("M.AssetType.Infograhic")).FirstOrDefault().Id.Value;
            long imageUsageIds = _taxonomyManager.AssetUsageEntities.Where(w => w.Identifier.Equals("CV.AssetUsage.Infograhic")).FirstOrDefault().Id.Value;
        ...
        private void UpdateAssetType(CovetrusAsset asset)
        {
            ...
            if (typeFolderLevel != null)
            {
                var assetId = typeFolderLevel switch
                {
                    ...
                    _ when typeFolderLevel.Equals("photography", StringComparison.InvariantCultureIgnoreCase) => _taxonomyManager.AssetTypeEntities.Where(w => w.Identifier.Equals("M.AssetType.ImageAnalysis")).FirstOrDefault().Id.Value,
