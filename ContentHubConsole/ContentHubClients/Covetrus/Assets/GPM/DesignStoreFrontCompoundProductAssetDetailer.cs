...
{
    public class DesignStoreFrontCompoundProductAssetDetailer : BaseDetailer
    {
        private const string GPM_CATALOG_NAME = "GPM Storefront Master Catalog";

        public DesignStoreFrontCompoundProductAssetDetailer(IWebMClient webMClient, ICollection<FileUploadResponse> fileUploadResponses) : base(webMClient, fileUploadResponses)
        {
        }
        ...
        public override async Task AssignToProduct(CovetrusAsset asset)
        {
            ...
            if (pathSplit.Any(a => a.Equals("Product Images")) && pathSplit.Any(a => a.Equals("Compounds")))
            {
                var productNumber = $"{pathSplit[4]}-{pathSplit[5]}";
